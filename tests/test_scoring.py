...
def test_value_score_prefers_actual_spend_and_is_bounded():
    assert scoring.value_score(9000, None, None) == 1.0
    assert scoring.value_score(18000, None, None) == 1.0  # capped, doesn't exceed 1
    assert 0 <= scoring.value_score(None, 40000, 150) <= 1.0
...
def test_higher_value_never_crosses_a_tier_boundary():
    ...
    low_value_waiting = scoring.TIER_SCORE["waiting_on_us"] + scoring.value_score(0, 0, 0) * 100
    high_value_new = scoring.TIER_SCORE["new"] + scoring.value_score(9000, 60000, 200) * 100
